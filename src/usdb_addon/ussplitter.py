...
CONFIG_DIR = Path(appdirs.user_data_dir("usdb_syncer", "bohning")).joinpath("addon_config")
CONFIGS = {}
...
def initialize_addon() -> None:
    ...
    if not config_file.exists():
        logger.error(f"Config file not found. Please create a config file at {config_file}")
        return
...
def write_song_tags(song_txt: Path, vocals: str, instrumental: str) -> bool:
    ...
    with open(song_txt, "r", encoding="utf-8") as f:
        for line in f.readlines():
            if not tags_added and (line.startswith(":") or line.startswith("*") or line.startswith("-")):
...
def on_download_finished(song: usdb_song.UsdbSong) -> None:
    ...
    response = requests.post(urljoin(CONFIGS["SERVER_URI"], "/split"), files={"audio": open(song_mp3, "rb")})
    ...
    while True:
        response = requests.get(urljoin(CONFIGS["SERVER_URI"], "/status"), params={"uuid": uuid})
        response.raise_for_status()
        status = response.text
        if status == "NONE":
            song_logger.error("An error occured while splitting. Server returned NONE status.")
            return
        elif status == "PENDING":
            pass
            ...
            song_logger.error("An error occured while splitting. Server returned ERROR status.")
            return
        time.sleep(5)
    
    response = requests.get(urljoin(CONFIGS["SERVER_URI"], "/result/vocals"), params={"uuid": uuid})
    response.raise_for_status()
    ...
    response = requests.get(urljoin(CONFIGS["SERVER_URI"], "/result/instrumental"), params={"uuid": uuid})
    response.raise_for_status()
    ...
    # Cleanup on server
    response = requests.post(urljoin(CONFIGS["SERVER_URI"], "/cleanup"), params={"uuid": uuid})
    response.raise_for_status()
